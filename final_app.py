...
st.markdown("Powered by **Random Forest Classifier** — an ensemble machine learning model.")
st.markdown("---")
...
if page == "Home":
    ...
    st.markdown("""
    This application uses a **Random Forest Classifier** to predict the likelihood
    of two diseases based on clinical and vocal measurements. A Random Forest works
    by building multiple decision trees and combining their outputs to produce a
    ...
    | Disease | Type | Input | Model Accuracy |
    |---------|------|-------|----------------|
    ...
    ### How to Use
    1. Select a disease module from the dropdown above
    2. Enter the required measurement values
    3. Click **Run Prediction**
    4. The app will display whether the disease is detected or not
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown(
        "This module uses a **Random Forest Classifier** trained on the UCI Parkinson's dataset. "
        "Enter the 22 vocal measurement features from the patient's voice recording below."
    )
    st.markdown("---")
    ...
    with col_info:
        st.caption("The default values above are dataset averages. Change them to patient-specific values.")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown(
        "This module uses a **Random Forest Classifier** trained on the UCI CKD dataset. "
        "Enter the patient's lab test results below."
    )
    st.markdown("---")
    ...
    if predict_btn:
        ...
        st.markdown("---")
        if prediction == 0:
            st.error("### Result: Chronic Kidney Disease Detected")
            st.metric("Confidence", f"{proba[0]*100:.1f}%")
        else:
            st.success("### Result: Healthy — No CKD Detected")
            st.metric("Confidence", f"{proba[1]*100:.1f}%")
